...
function computePeriodWindows(now = new Date()) {
  ...
  return {
    today: { key: localTodayKey(now), startsAt: startOfToday.toISOString(), endsAt: startOfNextDay.toISOString() },
    week: { key: localTodayKey(now), startsAt: weekStart.toISOString(), endsAt: startOfNextDay.toISOString() },
    month: { key: monthKey, startsAt: new Date(now.getFullYear(), now.getMonth(), 1).toISOString(), endsAt: startOfNextMonth.toISOString() }
  };
}
...
async function collectUsageOnce(options) {
  ...
  if (normalizedClients) {
    ...
    if (includesProma) {
      try {
        ...
        promaPeriods = {
          ...
          week: extractUsageFromTokscale(promaJson.week),
    ...
    if (anchorUsed) {
      ...
      week = anchor.week ? applyPeriodDelta(anchor.week, today, anchor.today) : emptyPeriod();
      ...
      const weekJson = await runTokscaleFn({ clients: tokscaleClients, flags: ['--week'], commandTimeoutMs });
    ...
    if (anchorUsed) {
      ...
      propagateTodayProjects(today, [week, month, allTime]);
    } else {
      decorateLocalPeriods({ today, week, month, allTime }, { retryMisses: true });
    }
    if (promaPeriods && !anchorUsed) {
      today = mergePeriods(today, promaPeriods.today);
      week = mergePeriods(week, promaPeriods.week || emptyPeriod());
  ...
  week = mergePeriods(windowsPeriods.week, wslBundle.week || emptyPeriod());
    ...
    try {
      ...
      if (saved && saved.dateKey === localTodayKey()) {
        ...
        if (saved.configFingerprint === fp) {
          anchor = { dateKey: saved.dateKey, today: saved.today, week: saved.week, month: saved.month, allTime: saved.allTime };
  ...
  async function performTick(reason, tickOptions = {}) {
    ...
    try {
      ...
      if (!anchored && captured) {
        anchor = { dateKey: todayKey, today: captured.windowsPeriods.today, week: captured.windowsPeriods.week || emptyPeriod(), month: captured.windowsPeriods.month, allTime: captured.windowsPeriods.allTime };
