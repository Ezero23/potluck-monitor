...
function isCacheStale(cache, now = Date.now()) {
  ...
  // Always refresh when the cache date is not today — the API updates daily,
  // so a yesterday cache means stale rates even if fetched recently.
  if (!cache.date || cache.date !== todayUtc(now)) return true;
  return false;
}
