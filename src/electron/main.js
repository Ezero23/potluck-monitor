...
// electron-updater must be required after the Electron app module is ready;
// defer initialization until the auto-updater is actually configured.
let autoUpdater = null;

if (!app.isPackaged) loadDotEnv();

const APP_NAME = 'Potluck Monitor';
...
function reportCredentialStorageError(context, error) {
  ...
  try {
    dialog.showErrorBox(
      ...
      `Potluck Monitor could not safely access credentials.json (${context}). The save was stopped and previous data was restored where possible. Check the file's JSON and permissions, then restart the app.\n\n${detail}`
    );
  } catch (_) {}
}
...
const RATE_REFRESH_HOUR = 9;
const RATE_REFRESH_MINUTE = 0;

function exchangeRateCachePath() {
  return path.join(app.getPath('userData'), 'exchange-rates.json');
}

function nextScheduledRateRefreshMs(now = Date.now()) {
  const d = new Date(now);
  d.setMilliseconds(0);
  d.setSeconds(0);
  d.setMinutes(RATE_REFRESH_MINUTE);
  d.setHours(RATE_REFRESH_HOUR);
  if (d.getTime() <= now) {
...
function rescheduleRateRefresh() {
  if (rateRefreshTimer) clearTimeout(rateRefreshTimer);
  rateRefreshTimer = setTimeout(() => {
    void refreshExchangeRates({ force: false });
    rescheduleRateRefresh();
  }, nextScheduledRateRefreshMs());
  rateRefreshTimer.unref?.();
}
...
async function writeExportTo(dir, periods, options = {}) {
  ...
  const files = exportFileSet({
    periods: periods || {},
    history,
    meta: { generatedAt: new Date().toISOString(), app: { name: 'potluck-monitor', version: appVersion() } }
  });
...
function isAllowedExternalUrl(value) {
  ...
  if (parsed.hostname === 'github.com' && parsed.pathname.startsWith('/Ezero23/potluck-monitor')) return true;
...
app.whenReady().then(() => {
  ...
  rescheduleRateRefresh();               // first refresh happens at next 09:00 local time
