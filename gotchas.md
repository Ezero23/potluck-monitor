...
- Duplicate-app cleanup must check the running executable, Spotlight results and LaunchServices registrations, including build bundles inside source backups. Checking only /Applications misses old versions that users can still launch. Preserve source and account data; remove duplicate executable bundles and verify the official app's embedded package version after reopening it.
