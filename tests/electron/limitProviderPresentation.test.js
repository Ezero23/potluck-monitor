...
test('limit percent tray mode renders provider icons into a generated tray image', () => {
  ...
  assert.match(main, /if \(process\.platform === 'darwin' && isGeneratedTrayIconMode\(id\)\) sized\.setTemplateImage\(true\)/);
});

test('provider tray icon delivery is race-guarded and renders plain template assets', () => {
  const app = readRendererFile('app.js');

  assert.match(app, /if \(!trayProviderIconDeliveryGuard\.isCurrent\(deliveryId\)\) return;/);
  assert.match(app, /providerImageToPngDataUrl\(img, 44\)/);
  assert.match(app, /function drawCustomTrayProviderImage/);
  assert.doesNotMatch(app, /showTrayProviderBadge/);
  assert.doesNotMatch(readRendererFile('index.html'), /trayProviderBadge|showTrayProviderBadgeInput/);
});
