...
test('defaultHomeModulePreferences keeps the full Home module order and hides new modules', () => {
  assert.equal(DEFAULT_HOME_MODULE_ORDER, 'weekly,limits,tool,device,model,trends');
  assert.deepEqual(defaultHomeModulePreferences(), {
    homeModuleOrder: 'weekly,limits,tool,device,model,trends',
...
test('normalizeHomeModuleOrder drops invalid ids and appends missing modules', () => {
  assert.deepEqual(
    ...
    ['device', 'limits', 'weekly', 'tool', 'model', 'trends']
  );
});
...
test('normalizeHiddenHomeModules keeps known ids but never hides every Home module', () => {
  ...
  assert.equal(normalizeHiddenHomeModules('weekly,limits,tool,device,model,trends', modules), '');
});
...
test('moveHomeModuleOrder and reorderHomeModuleOrder update saved order', () => {
  assert.equal(
    moveHomeModuleOrder('weekly,limits,tool,device,model,trends', modules, 'device', 'up'),
    'weekly,limits,device,tool,model,trends'
  );
  assert.equal(
    reorderHomeModuleOrder('weekly,limits,tool,device,model,trends', modules, 'trends', 1),
    'weekly,trends,limits,tool,device,model'
  );
});
