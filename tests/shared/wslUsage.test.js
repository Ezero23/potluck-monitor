...
test('collectWslUsage passes every requested client to each discovered home', async () => {
  ...
  for (const home of ['\\\\wsl$\\Ubuntu\\home\\alice', '\\\\wsl$\\Ubuntu\\home\\carol']) {
    assert.deepEqual(seenClientsPerHome[home], ['claude,zed', 'claude,zed', 'claude,zed', 'claude,zed']);
  }
});
...
test('collectWslUsage sums two homes per period', async () => {
  ...
  const map = {
    '\\\\wsl$\\Ubuntu\\home\\alice': { today: 10, week: 20, month: 100, allTime: 1000 },
    '\\\\wsl$\\Ubuntu\\home\\bob': { today: 5, week: 10, month: 50, allTime: 500 }
  };
